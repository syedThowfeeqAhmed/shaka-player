...
shaka.media.DrmEngine = class {
    ...
    if (!mediaKeySystemAccess) {
      ...
      try {
        const evt = new CustomEvent(
            'shakaCustomEvent_DRM_Session_Acquired');
        window.dispatchEvent(evt);
      } catch (error) {
        shaka.log.v2(error, 'raising custom event ',
            'Requesting failed with config');
      }
    }
    this.destroyer_.ensureNotDestroyed();
  ...
  createSession(initDataType, initData, sessionType) {
    ...
    try {
      ...
      try {
        const evt = new CustomEvent(
            'shakaCustomEvent_DRM_New_Session_created', {
              detail: {sessionType: sessionType}});
        window.dispatchEvent(evt);
      } catch (error) {
        shaka.log.v2(error, 'raising custom event ',
            'shakaCustomEvent_DRM_New_Session_created',
  ...
  onSessionMessage_(event) {
    // On first event we can assume DRM Keys Loaded
    try {
      const evt = new CustomEvent(
          'shakaCustomEvent_DRM_Keys_Loaded', {
            detail: {event: event}});
      window.dispatchEvent(evt);
    } catch (error) {
      shaka.log.v2(error, ' raising custom event ',
          'shakaCustomEvent_DRM_Keys_Loaded failed at event', event);
    }
  ...
  async sendLicenseRequest_(event) {
    ...
    try {
      const evt = new CustomEvent(
          'shakaCustomEvent_DRM_License_Request', {
    ...
    try {
      const evt = new CustomEvent(
          'shakaCustomEvent_DRM_License_Response', {
            detail: {request: request}});
      window.dispatchEvent(evt);
    } catch (error) {
      shaka.log.v2(error, ' raising custom event ',
